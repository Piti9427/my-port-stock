...
- Current price must pass the `Price Source Ladder` and `Current Price Acceptance Gate` in `GEMINI.md` / `ELITE_INVESTOR_SOP.md`.
...
To prevent crashes and bugs when upgrading or using the latest packages in this repository, always adhere to the following package-specific rules:

- **Clerk React v6 (Core 3)**: The `<SignedIn>` and `<SignedOut>` components have been deprecated. Always use `<Show when="signed-in">` and `<Show when="signed-out">` for conditional auth rendering in the frontend.
- **Clerk Express**: The `req.auth` property is a function, not an object. Always use `const { getAuth } = require('@clerk/express'); getAuth(req).userId` to access the authenticated user's ID safely in backend routes.
- **Supabase JS v2**: The backend uses CommonJS (`require`). Always instantiate the client using `const { createClient } = require('@supabase/supabase-js')` and DO NOT use ES Modules `import/export` syntax in the backend unless the `package.json` explicitly declares `"type": "module"`.
