...
test("live migration is additive and does not drop runtime data tables", () => {
  ...
  assert.match(
    migration,
    /ALTER TABLE public\.journal ADD COLUMN IF NOT EXISTS import_batch_id/i,
  );
  assert.match(
    migration,
    /ALTER TABLE public\.watchlists ADD COLUMN IF NOT EXISTS import_batch_id/i,
  );
  assert.match(
    migration,
    /ALTER TABLE public\.holdings ENABLE ROW LEVEL SECURITY/i,
  );
...
test("backend integrity migration adds guarded constraints without rewriting data", () => {
  ...
  assert.match(
    migration,
    /ADD CONSTRAINT holdings_avg_cost_nonnegative_check/i,
  );
  assert.match(migration, /CHECK \(avg_cost >= 0\)/i);
  assert.match(
    migration,
    /ADD CONSTRAINT watchlists_alert_type_allowed_check/i,
  );
  assert.match(migration, /CHECK \(alert_type IN \('above', 'below'\)\)/i);
  assert.match(migration, /IF NOT EXISTS[\s\S]+pg_constraint/i);
  assert.match(
    migration,
    /RAISE EXCEPTION 'Cannot add watchlists import batch foreign key/i,
  );
...
test("canonical schema snapshots contain user_preferences table, RLS, and grants", () => {
  ...
  for (const candidate of [schema, rootSchema]) {
    assert.match(
      candidate,
      /CREATE TABLE IF NOT EXISTS public\.user_preferences/i,
    );
    assert.match(
      candidate,
      /CONSTRAINT user_preferences_currency_check\s+CHECK\s*\(reporting_currency\s+IN\s*\('THB',\s*'USD'\)\)/i,
    );
    assert.match(
      candidate,
      /CONSTRAINT user_preferences_disclosure_check\s+CHECK\s*\(disclosure_level\s+IN\s*\('beginner',\s*'advanced'\)\)/i,
    );
    assert.match(
      candidate,
      /CONSTRAINT user_preferences_theme_check\s+CHECK\s*\(theme\s+IN\s*\('dark',\s*'light',\s*'system'\)\)/i,
    );
    assert.match(
      candidate,
