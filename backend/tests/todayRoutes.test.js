...
describe('Today API Route — /api/today', () => {
  ...
  it('GET /api/today returns queue and pulse objects for authenticated user', async () => {
    ...
    assert.ok('missingStopCount' in res.body.pulse, 'pulse should have missingStopCount');
  });
  ...
  it('queue items are ordered: protect > prepare > opportunity > learn', async () => {
    ...
    const sevenDaysAgo = new Date(today - 20 * 24 * 60 * 60 * 1000).toISOString();

    // Swing trade held 20 days → time stop (Prepare)
    mockHoldings = [{ ticker: 'AAPL', shares: 10, avg_cost: 150, sector: 'Technology' }];
    mockJournal = [
      {
        ticker: 'AAPL',
        status: 'OPEN',
    ...
    mockJournal.push({ ticker: 'TSLA', status: 'CLOSED', notes: '', mode: 'Swing Trade' });

    // Price for MSFT triggers above-alert (200+)
    mockEnriched = [
      { ticker: 'AAPL', shares: 10, avg_cost: 150, price: 155, change: 0, changePct: 0, beta: 1.0, earningsTimestamp: null, spark: [], sector: 'Technology', stop_loss: 100, mode: 'Swing Trade', opened_at: sevenDaysAgo },
      { ticker: 'MSFT', alert_price: 200, alert_type: 'above', price: 210, change: 0, changePct: 0, beta: 1.0, earningsTimestamp: null, spark: [] },
    ];
    ...
    if (prepareIdx !== -1 && opportunityIdx !== -1) {
      assert.ok(prepareIdx < opportunityIdx, 'prepare should come before opportunity');
    }
    if (opportunityIdx !== -1 && learnIdx !== -1) {
      assert.ok(opportunityIdx < learnIdx, 'opportunity should come before learn');
    }
  });
  ...
  it('generates a protect item when stop_loss is missing for an open trade', async () => {
    mockHoldings = [{ ticker: 'NVDA', shares: 5, avg_cost: 400, sector: 'Technology' }];
    mockJournal = [
      { ticker: 'NVDA', status: 'OPEN', mode: 'Swing Trade', stop_loss: null, notes: '', opened_at: new Date().toISOString() },
    ];
    ...
    assert.ok(protectItems.length > 0, 'should have at least one protect item for missing stop');
  });

  it('generates a protect item when thesis has #failed tag in notes', async () => {
    mockHoldings = [{ ticker: 'META', shares: 3, avg_cost: 500, sector: 'Communication Services' }];
    mockJournal = [
      { ticker: 'META', status: 'OPEN', mode: 'Long-Term/Core', stop_loss: 400, notes: 'Thesis is broken. #failed guidance miss', opened_at: new Date().toISOString() },
    ];
    ...
    assert.equal(res.statusCode, 200);
    const protectItems = res.body.queue.filter((c) => c.category === 'protect');
    assert.ok(protectItems.length > 0, 'should have protect item for thesis failure');
  ...
  it('generates a learn item for closed trade with no notes', async () => {
    ...
    assert.ok(learnItems.length > 0, 'should have a learn post-mortem reminder');
  });
});
