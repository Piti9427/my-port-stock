...
const userId = process.argv.find((arg) => arg.startsWith('--user='))?.slice('--user='.length) || process.env.MARKDOWN_IMPORT_OWNER_USER_ID;
...
async function main() {
  ...
  if (!shouldWrite) {
    const portfolioMarkdown = fs.readFileSync(path.join(root, 'stock_portfolio.md'), 'utf8');
    const journalMarkdown = fs.readFileSync(path.join(root, 'trade_journal.md'), 'utf8');
    ...
    console.log(JSON.stringify({
      ...
      watchlist_tickers: payload.watchlistItems.map((entry) => entry.ticker),
    }, null, 2));
    return;
  }
