...
const { createScopedClient, supabaseConfigured, supabaseUrl } = require('./src/db/supabaseClient');
...
async function testConnection() {
  ...
  console.log(`1. Generating Scoped Supabase Client for user: ${mockUserId}...`);
  
  const client = createScopedClient(mockUserId);
  if (!client) {
    console.error("❌ Failed to create scoped client. Check SUPABASE_JWT_SECRET configuration.");
    process.exit(1);
  }
  ...
  if (error) {
    ...
    console.log("✅ เชื่อมต่อ Supabase สำเร็จด้วย Clerk JWT และผ่าน RLS เรียบร้อย!");
    console.log(`✅ พบตาราง 'holdings' พร้อมใช้งาน (พบข้อมูล ${data.length} แถว)`);
  }
}
