...
function parsePortfolioHoldings(markdown, userId) {
  const rows = [];
  const tableLine =
    /^\|\s*\*\*\$(?<ticker>[A-Z0-9.-]+)\*\*\s*\|\s*(?<shares>[0-9.]+)\s*\|\s*\$(?<avgCost>[0-9.,]+)\s*\|/;
...
function withImportMetadata(rows, metadata) {
  return rows.map((row) => ({
    ...
    source_hash: sha256(
      `${metadata.sourceFile}|${metadata.sourceSection}|${row.ticker}|${JSON.stringify(row)}`,
    ),
...
function parseJournalTrades(markdown, userId) {
  const rows = [];
  const tableLine =
    /^\|\s*(?<date>\d{4}-\d{2}-\d{2})\s*\|\s*\$(?<ticker>[A-Z0-9.-]+)\s*\|\s*(?<type>[^|]+)\|\s*\$(?<entry>[0-9.,]+)\s*\|\s*\$(?<stop>[0-9.,]+)\s*\|\s*\$(?<target>[0-9.,]+)\s*\|\s*(?<rr>[^|]+)\|\s*(?<thesis>[^|]+)\|\s*(?<status>[^|]+)\|/;
  ...
  for (const line of markdown.split("\n")) {
    ...
    let shares = 1;
    const sharesMatch = match.groups.thesis.match(
      /\$?[0-9.,]+\s*\/\s*(?<qty>[0-9.]+)\s*shares?/i,
    );
    if (sharesMatch) {
    ...
    rows.push({
      ...
      status:
        match.groups.status.trim().toUpperCase() === "ACTIVE"
          ? "OPEN"
          : "CLOSED",
...
function parseWatchlist(markdown, userId) {
  const rows = [];
  const tableLine =
    /^\|\s*\*\*\$(?<ticker>[A-Z0-9.-]+)\*\*\s*\|\s*(?<sector>[^|]+)\|\s*(?<entryZone>[^|]+)\|\s*(?<target>[^|]+)\|\s*(?<hypothesis>[^|]+)\|/;

  // Extract only the Tactical Watchlist section to prevent pulling from holdings snapshot
  const sections = markdown.split(/^##\s+/m);
  const watchlistSection = sections.find((s) =>
    s.trim().startsWith("Tactical Watchlist"),
  );
...
function assertMarkdownImportOwner(userId, env = process.env) {
  ...
  if (!ownerUserId) {
    throw new Error(
      "MARKDOWN_IMPORT_OWNER_USER_ID is required for markdown import",
    );
  }
...
function buildMarkdownImportPayload({
  ...
  const journalTrades = withImportMetadata(
    parseJournalTrades(journalMarkdown, userId),
    {
      importBatchId,
  ...
  const watchlistItems = withImportMetadata(
    parseWatchlist(portfolioMarkdown, userId),
    {
      importBatchId,
  ...
  for (const holding of holdings) {
    const matchingTrades = journalTrades.filter(
      (trade) => trade.ticker === holding.ticker,
    );
    ...
    if (Math.abs(tradeShares - holding.shares) > 0.0001) {
      finalJournalEntries.push({
        ...
        source_hash: sha256(
          `stock_portfolio.md|Holdings Snapshot|${holding.ticker}|${JSON.stringify(holding)}`,
        ),
...
async function importMarkdownSnapshotForOwner(userId) {
  ...
  let journalMarkdown;
  try {
    portfolioMarkdown = fs.readFileSync(
      path.join(root, "stock_portfolio.md"),
      "utf8",
    );
    journalMarkdown = fs.readFileSync(
  ...
  if (existingBatch && existingBatch.length > 0) {
    return {
      migrated: false,
      reason: "Import batch already exists",
  ...
  try {
    ...
    const { error: updateBatchErr } = await scopedSupabase
      ...
      .update({
        status: "succeeded",
        inserted_journal: insertedJournal,
        inserted_watchlists: insertedWatchlists,
      })
      .eq("import_batch_id", payload.batch.import_batch_id);
