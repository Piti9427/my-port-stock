...
describe('MetricCard Component (Tailwind)', () => {
  ...
  it('renders positive change badge with emerald profit classes', () => {
    ...
    expect(changeBadge.className).toContain('text-emerald-400');
  });

  it('renders negative change badge with rose loss classes', () => {
    render(<MetricCard label="Drawdown" value="-$1,100" change="-1.20%" changeType="negative" />);
    const changeBadge = screen.getByText('-1.20%');
    expect(changeBadge).toBeDefined();
    expect(changeBadge.className).toContain('text-rose-400');
  });
