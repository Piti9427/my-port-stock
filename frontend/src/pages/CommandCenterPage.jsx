...
export default function CommandCenterPage() {
  ...
  const statusColor = analysisResult?.status === 'green' ? 'var(--fin-success)' :
                      analysisResult?.status === 'yellow' ? 'var(--fin-warning)' :
                      analysisResult?.status === 'red' ? 'var(--fin-danger)' : 'gray';

  return (
    <div className="command-center-page" style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: '16px', padding: '16px' }}>
      
      <div style={{ display: 'flex', gap: '16px', flex: 1, minHeight: 0 }}>
        
        {/* Left Pane: Live Data Feed */}
        <div className="glass-panel" style={{ width: '350px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto' }}>
          ...
          <form onSubmit={handleSearch} style={{ display: 'flex', gap: '8px' }}>
            ...
            />
            <button type="submit" className="btn btn-primary" style={{ padding: '8px 16px', borderRadius: '4px' }}>Search</button>
          </form>
          
          {fetchError && <div style={{ color: 'var(--fin-danger)' }}>{fetchError}</div>}
          
          {quoteData && (
            <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '12px', borderRadius: '8px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <strong style={{ color: quoteData.current_price_acceptance_gate === 'pass' ? 'var(--fin-success)' : 'var(--fin-danger)' }}>
          ...
          <div style={{ marginTop: 'auto' }}>
            <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold' }}>Decision Mode</label>
            <select 
              className="input-field" 
              value={decisionMode} 
              onChange={e => setDecisionMode(e.target.value)} 
              style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid var(--border-color)', marginBottom: '16px' }}
            >
            ...
            <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold' }}>Manual Price Override</label>
            <input 
              className="input-field" 
              type="number"
              step="0.01"
        ...
        <div className="glass-panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div className="glass-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div style={{ fontSize: '1.1rem' }}>Score: <strong>{analysisResult?.score || '-'}</strong></div>
            </div>
          ...
          {/* AI Verdict Summary */}
          {analysisResult && (
            <div className="glass-card verdict-section" style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: '16px', background: 'rgba(0,0,0,0.2)' }}>
              <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>
                Verdict: <span style={{ color: analysisResult.verdict?.toLowerCase().includes('buy') ? 'var(--fin-success)' : 'var(--text-primary)' }}>{analysisResult.verdict}</span>
              </div>
              <div style={{ fontSize: '1rem' }}><strong>Reason:</strong> {analysisResult.one_line_reason}</div>
              <div style={{ fontSize: '1rem' }}><strong>Next Action:</strong> {analysisResult.immediate_next_action}</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="glass-panel" style={{ display: 'flex', gap: '16px', justifyContent: 'flex-end', padding: '16px' }}>
        <button 
          className="btn btn-success" 
          disabled={!analysisComplete}
          style={{ padding: '8px 24px', fontWeight: 'bold', background: analysisComplete ? 'var(--fin-success)' : 'gray', color: 'white', border: 'none', borderRadius: '4px', cursor: analysisComplete ? 'pointer' : 'not-allowed' }}
        >
          Buy
        </button>
        <button 
          ...
          disabled={!analysisComplete}
          style={{ padding: '8px 24px', fontWeight: 'bold', background: analysisComplete ? 'var(--fin-danger)' : 'gray', color: 'white', border: 'none', borderRadius: '4px', cursor: analysisComplete ? 'pointer' : 'not-allowed' }}
        >
          Sell
        </button>
        <button 
          ...
          disabled={!analysisComplete}
          style={{ padding: '8px 24px', fontWeight: 'bold', background: analysisComplete ? '#4b5563' : 'gray', color: 'white', border: 'none', borderRadius: '4px', cursor: analysisComplete ? 'pointer' : 'not-allowed' }}
        >
          Add to Watchlist
        </button>
      </div>
